...
public class FieldsPreprocessor {
	...
	private static Logger log = LoggerFactory.getLogger(org.docx4j.model.fields.FieldsPreprocessor.class);		
...
//		fieldRPr = null;
		...
		if(log.isDebugEnabled()) {
            log.debug(XmlUtils.marshaltoString(p,true));
        }
	...
	private void handleContent(List<Object> objects, ContentAccessor attachmentPoint) {
		...
		for (Object o : objects ) {
			...
			if ( o instanceof R ) {
				...
				R existingRun = (R)o;
				if (existingRun.getRPr() != null)
				{
				    lastR = existingRun;
				    if (newR.getRPr() == null)
				    {
					newR.setRPr(lastR.getRPr());
				    }
				}
...
//			}
	...
	//						R separateR = createR();							
				...
				if (o instanceof Text && ((Text) o).getValue().trim().isEmpty()) {
					...
					/**
					 * Имеем поток instrText 
					 * Сначала пробел, затем DOCPROPERTY, затем пробел, затем идет инструкция.
					 * Каждая часть - отдельным тегом. Если пропускать пробелы, то 
					 * DOCPROPERTY сливается с инструкцией.
					 * Если оставить первый пробел, далее FieldRef не определяет правильно имя поля.
					 * Так как использует первую инструкцию.
					 * Пробное решение - вместо continue всегда на пробеле, делаем его только 
					 * на первом. Если это пробел, а других инструкций еще нет, 
					 * то есть убираем ведущие пробелы.
					 */
					if (currentField.getInstructions().size() == 0)
					continue;
				}
